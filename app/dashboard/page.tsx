...
import Image from 'next/image'; // Masih dipakai untuk ikon statis/avatar
...
export default function DashboardPage() {
  ...
  // GAMBAR DEFAULT (Agar tidak crash jika user belum upload)
  const DEFAULT_IMAGE = "https://images.unsplash.com/photo-1511285560982-1351cdeb9821?q=80&w=800";
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Update State
        setData({
          userName: invData ? invData.groom_name.split(' ')[0] : "Pengantin",
          activeProject: {
            ...
            // FIX: Gunakan DEFAULT_IMAGE jika cover_photo kosong/null
  ...
  if (isLoading) {
    return (
      <div className="space-y-8 p-4 animate-pulse">
        ...
        <div className="h-64 bg-gray-200 rounded-[2.5rem] mt-8"></div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 animate-fade-in-up font-sans text-gray-800">
      ...
      {/* --- 2. STATS CARDS (CLEAN LUXURY) --- */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
          {/* Card 1: Total Tamu */}
          <div 
            onClick={goToGuests}
            className="group relative bg-white rounded-[2.5rem] p-8 shadow-sm border border-pink-100 hover:shadow-xl hover:border-pink-200 transition-all duration-500 overflow-hidden h-56 flex flex-col justify-end cursor-pointer"
          >
              ...
              <div className="relative z-10">
                  ...
                  <div className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-pink-50 text-pink-600 border border-pink-100">
                      Terdaftar
                  </div>
              </div>
          </div>
          ...
          <div className="group relative bg-white rounded-[2.5rem] p-8 shadow-sm border border-emerald-100 hover:shadow-xl hover:border-emerald-200 transition-all duration-500 overflow-hidden h-56 flex flex-col justify-end cursor-default">
              <div className="absolute -bottom-6 -right-6 opacity-40 rotate-12 group-hover:rotate-0 group-hover:opacity-40 group-hover:scale-110 transition-all duration-500 ease-out text-emerald-600">
                  <svg className="w-40 h-40" viewBox="0 0 24 24" fill="currentColor" strokeWidth="1">
                      <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
              </div>
              <div className="relative z-10">
                  ...
                  <div className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">
                      {data.stats.totalGuests > 0 ? Math.round((data.stats.confirmed / data.stats.totalGuests) * 100) : 0}% Confirm
                  </div>
              </div>
          </div>
          ...
          <div className="group relative bg-white rounded-[2.5rem] p-8 shadow-sm border border-orange-100 hover:shadow-xl hover:border-orange-200 transition-all duration-500 overflow-hidden h-56 flex flex-col justify-end cursor-default">
              <div className="absolute -bottom-6 -right-6 opacity-40 rotate-12 group-hover:rotate-0 group-hover:opacity-40 group-hover:scale-110 transition-all duration-500 ease-out text-orange-600">
                  <svg className="w-40 h-40" viewBox="0 0 24 24" fill="currentColor" strokeWidth="1">
                       <path d="M6 2v6h.01L6 8.01 10 12l-4 4 .01.01H6V22h12v-5.99h-.01L18 16l-4-4 4-3.99-.01-.01H18V2H6zm10 14.5V20H8v-3.5l4-4 4 4z" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
              </div>
              <div className="relative z-10">
                  ...
                  <div className="mt-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-orange-50 text-orange-600 border border-orange-100">
                      Menunggu
                  </div>
              </div>
          </div>
      ...
      {/* --- 3. ACTIVE PROJECT (DATA REAL DENGAN SAFE IMAGE) --- */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-3 shadow-sm border border-gray-100 flex flex-col md:flex-row group hover:border-pink-100 transition-colors">
              <div onClick={openPreview} className="w-full md:w-2/5 h-64 md:h-auto relative rounded-4xl overflow-hidden cursor-pointer">
                  {/* FIX: Ganti Next Image dengan <img> biasa agar aman dari error hostname */}
                  <img 
              ...
              <div className="flex-1 p-6 md:p-8 flex flex-col justify-center">
                  ...
                  <div className="mt-8 pt-8 border-t border-gray-50 grid grid-cols-2 gap-4">
                      <button onClick={goToEdit} suppressHydrationWarning={true} className="flex items-center justify-center gap-2 w-full py-4 bg-gray-900 text-white rounded-2xl font-bold text-sm hover:bg-gray-800 transition shadow-lg hover:shadow-gray-200">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                          Edit Konten
                      </button>
                      <button onClick={openPreview} suppressHydrationWarning={true} className="flex items-center justify-center gap-2 w-full py-4 bg-white border border-gray-200 text-gray-700 rounded-2xl font-bold text-sm hover:border-pink-500 hover:text-pink-600 transition">
          ...
          <div className="lg:col-span-1 bg-linear-to-br from-pink-600 to-purple-700 rounded-[2.5rem] p-8 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl">
              ...
              <button onClick={handleUpgrade} suppressHydrationWarning={true} className="relative z-10 mt-8 w-full bg-white text-pink-700 py-3.5 rounded-xl font-bold text-sm hover:bg-pink-50 transition shadow-lg">

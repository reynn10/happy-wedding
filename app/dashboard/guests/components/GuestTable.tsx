...
export default function GuestTable({ 
  ...
  return (
    <>
      {/* FILTER BAR */}
      <div className="bg-white p-2 rounded-full border border-gray-200 shadow-sm flex items-center pl-6 mb-6">
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden min-h-[300px] flex flex-col">
          {isLoading ? (
            ...
            // FIX: Wrapper untuk Scroll Horizontal di HP
            <div className="overflow-x-auto w-full">
                {/* FIX: min-w-[800px] agar tabel tidak gepeng di HP, user harus scroll */}
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead className="bg-stone-50/50">
                      <tr className="text-xs text-gray-400 uppercase tracking-widest border-b border-gray-100">
                          ...
                          <th className="pb-4 pr-8 font-semibold text-right pt-6">Aksi</th>
                      </tr>
                  </thead>
                          ...
                          <tr key={guest.id} className={`group transition-colors ${selectedIds.includes(guest.id) ? 'bg-pink-50/50' : 'hover:bg-stone-50'}`}>
                              ...
                              <td className="py-4 pl-2">
                                  <div className="flex items-center gap-3">
                                      <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center text-gray-500 font-bold text-xs shrink-0">
                              ...
                              <td className="py-4 text-center">
                                  <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider 
                                      ${guest.status === 'Hadir' ? 'bg-green-100 text-green-700' : 
                                        guest.status === 'Tidak Hadir' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                      {guest.status}
                                  </span>
                              </td>
